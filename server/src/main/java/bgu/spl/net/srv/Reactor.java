...
public class Reactor<T> implements Server<T> {
	...
	selectorThread = Thread.currentThread();
        try (Selector selector = Selector.open();
            ...
            while (!Thread.currentThread().isInterrupted()) {
                ...
                //also, to not submit the task twice
    ...
    private void handleAccept(ServerSocketChannel serverChan, Selector selector) throws IOException {
        ...
        final NonBlockingConnectionHandler<T> handler = new NonBlockingConnectionHandler<>(
                ...
                clientChan,
                this);
        int id = nextId.incrementAndGet();//generate connection id for every client
        connections.connect(id, handler);

        // we want to prevent reading or processing before protocol start is done
        clientChan.register(selector, 0, handler);

        pool.submit(handler, () -> {
            protocol.start(id, connections);
            // now it's safe to start reading
