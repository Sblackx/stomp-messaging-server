...
public class Database {
	...
	private String executeSQL(String sql) {
		synchronized (this) {
			try (Socket socket = new Socket(sqlHost, sqlPort);
				...
				// System.out.println("Sending SQLLL: "+sql); debug
				out.print(sql + '\0');
				out.flush();
	...
	public LoginStatus login(int connectionId, String username, String password) {
		...
		// need to check if user already exist in the table, if it is return WrongPass
		String sql = String.format(
				"SELECT password FROM users WHERE username='%s'",
				escapeSql(username));
		String Query = executeSQL(sql);
		if (Query.startsWith("SUCCESS|(")) {
			String[] tuples = Query.split("\\|");
			String pass = tuples[1].replace("(", "").replace(")", "").replace("'", "").trim();

			String storedPass = ((pass.split(","))[0]).trim();
	...
	private LoginStatus userExistsCase(int connectionId, String username, String password) {// hold the user login, the
																							// user already have been
																							// regestred before
		User user = userMap.get(username);
		synchronized (user) {
			if (user.isLoggedIn()) {
